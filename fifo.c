...
void* playback()
{
    ...
    while(1) {
        ...
        if (sz <  chunk * sizeof(short) * outchannels)
        {
             usleep(1000);
             continue;
        }
...
void* capture()
{
    ...
    while(1) {
        ...
        if (sz >=  chunk * sizeof(short) * inchannels * capturechunks)
        {
             usleep(1000);
             continue;
        }
...
int main(int argc, char** argv)
{
    ...
    printf("OpenSLES hardware opened %d %d in, %d out, %d chunk size, %d capture chunks.\n",
